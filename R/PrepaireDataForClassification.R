...
PrepaireDataForClassification <- function(DataToProcess, Scaling = "none", SplitRatio = 0.67,
  nNestings = 5, SeparateValidationRatio = 0, SeedsAndNestings) {

  ### Functions Scaling function
  ScaleData <- function(Data, Scaling) {
    switch(Scaling, z = {
      cbind.data.frame(Classes = Data$Classes, apply(Data[, 2:ncol(Data)],
        2, function(x) scale(x, center = T, scale = T)))
    }, range = {
  ...
  SplitTrainingTest <- function(Data, Seed) {
    ...
    TrainDataScaledPermuted <- cbind.data.frame(Classes = TrainDataScaled$Classes,
  ...
  # Nesting function
  CreateNestedData <- function(Data, DataToProcessScaledSeparateValidation, nNestings, SeedsAndNestings) {

    # Mark data set for nesting
    list.of.nestings.perCls <- unlist(lapply(unique(Data$Classes), function(x) {
      set.seed(SeedsAndNestings[1,1])
    ...
    DataToProcessNested <- lapply(1:nNestings, function(i) {
      ...
      if (nNestings > 1) {
        DataToProcessNestedTrainingTest <- DataToProcessScaledNestingsMarked[DataToProcessScaledNestingsMarked$list.of.nestings.perCls != i, ]
      } else {
        DataToProcessNestedTrainingTest <- DataToProcessScaledNestingsMarked[DataToProcessScaledNestingsMarked$list.of.nestings.perCls == i, ]
      }
      DataToProcessNestedValidation <- DataToProcessScaledNestingsMarked[DataToProcessScaledNestingsMarked$list.of.nestings.perCls == i, ]
      DataToProcessNestedTrainingTest <- DataToProcessNestedTrainingTest[,
        -ncol(DataToProcessNestedTrainingTest)]
      DataToProcessNestedValidation <- DataToProcessNestedValidation[, -ncol(DataToProcessNestedValidation)]
  ...
  AssembleNestedDataSets <- function(DataToProcessNested, SeparateValidationDataScaled,
    ...
    sample <- caTools::sample.split(DataToProcessNested[[Nesting]]$DataToProcessNestedTrainingTest$Classes,
      SplitRatio = 0.67)
    TrainDataScaled <- subset(DataToProcessNested[[Nesting]]$DataToProcessNestedTrainingTest,
      sample == TRUE)
    TestDataScaled <- subset(DataToProcessNested[[Nesting]]$DataToProcessNestedTrainingTest,
      sample == FALSE)

    # Select the validation sample, either defined during nested cross
    # validation or place the test data as validation data
    if (nNestings > 1) {
      ValidationDataScaled <- DataToProcessNested[[Nesting]]$DataToProcessNestedValidation
    } else {
    ...
    TrainDataScaledPermuted <- cbind.data.frame(Classes = TrainDataScaled$Classes,
      apply(TrainDataScaled[-1], 2, sample))
    TestDataScaledPermuted <- cbind.data.frame(Classes = TestDataScaled$Classes,
      apply(TestDataScaled[-1], 2, sample))
    ValidationDataScaledPermuted <- cbind.data.frame(Classes = ValidationDataScaled$Classes,
      apply(ValidationDataScaled[-1], 2, sample))
    SeparateValidationDataScaledPermuted <- cbind.data.frame(Classes = SeparateValidationDataScaled$Classes,
  ...
  DataToProcessScaledDownsampled <- opdisDownsampling::opdisDownsampling(Data = DataToProcessScaled[, -1],
    Cls = DataToProcessScaled$Classes, Size = length(DataToProcess$Classes) *
    (1 - SeparateValidationRatio), Seed = Seeds[1])
  DataToProcessScaled <- cbind.data.frame(Classes = DataToProcessScaledDownsampled$ReducedData$Cls,
    DataToProcessScaledDownsampled$ReducedData[, -ncol(DataToProcessScaledDownsampled$ReducedData)])
  if (SeparateValidationRatio > 0) {
    DataToProcessScaledSeparateValidation <- cbind.data.frame(Classes = DataToProcessScaledDownsampled$RemovedData$Cls,
      DataToProcessScaledDownsampled$RemovedData[, -ncol(DataToProcessScaledDownsampled$RemovedData)])
  } else {
    DataToProcessScaledSeparateValidation <- data.frame(matrix(NA, nrow = 0,
      ncol = ncol(DataToProcessScaledDownsampled$ReducedData)))
    names(DataToProcessScaledSeparateValidation) <- names(DataToProcessScaled)
  }
  ...
  PrepairedData <- lapply(seq(Seeds), function(x) {
    AssembleNestedDataSets(DataToProcessNested = DataToProcessNested, SplitRatio = SplitRatio,
      Seed = list.of.seeds.nestings[1, x], Nesting = list.of.seeds.nestings[2,
        x], nNestings = nNestings)
  })
